...
>
> This document records research completed on the `ctxmap-harness` branch and last
> updated **2026-07-11**. Its **results are accurate and unamended**. Its
> forward-looking statements — the header's "migration is underway", §6's open
> decisions and roadmap — have since resolved, several differently than predicted.
> This block is the reconciliation; the body below is left as written.
>
...
>
> **The fact store is attached but hollow.** `turnengine.NewManager` →
> `attachContextEngine` runs by default, but at **v1 = working-state only**:
> `store.Open(":memory:")` (no persistence across restarts) and
> Proposer/Embedder/PairJudge all nil. `cmd/agora/engine.go`'s
> `WithContextExtraction(true)` wires the **active API model** as extractor, not the
> local Qwen pair this document describes. The Embedder stays nil deliberately, so
> `reconcileScan` short-circuits to token-overlap matching before ever consulting
> the PairJudge — **the JudgePair seam is dormant.** Injection attaches only for
> `CategoryDirectAPI`; the claudesdk lane is excluded after a live 2026-07-21
> incident where the memory block shadowed the operator's own text.
>
> **Therefore: §3's dialogue results have never run outside this branch.** They are
> a validated bench result, not a banked operational one. Do not plan against them
> as though the mechanism is live.
>
> **§6's open decisions, resolved**
>
> - *Module boundary* → separate `bridle/ctxmap` packages, hooks-only attachment.
> - *cgo/model-weight footprint* → build-tag gated (`ctxmap_llama`). agora main
>   needs no tag, because it runs no local models.
> - *Store scope in a fleet* → **not** the "lean: per-repo" this document favours.
>   agora is per-Manager and in-memory; `nexus/runtime/ctxmapwire` is per-aspect and
>   in-process. A shared service was never built.
> - *Rollout ("one aspect soaked, then fleet-wide")* → the soak ran to
>   1 fact / 0 links (2026-07-16) and stopped. The nexus fleet is inert: CI builds
>   untagged binaries, so `ctxmapwire` resolves to `Nop()`.
>
> **Reviving the bench is a port, not a merge.** `ctxbench` imports
> `internal/harness`, `internal/bench` and `internal/backend/openai` — **all three
> were deleted from main.** Its `bridle/ctxmap/*` imports are still valid. Budget a
> rewrite against `turnengine`, not a cherry-pick.
>
> **Corrections to this document's own model recommendations** (measured 2026-07-29,
> CPU, 22 discriminating probes): §3's rejection of Gemma 4 for **extraction**
> stands (P 55 / R 71 vs Qwen's P 74–79 / R 84–87). But in the **pair-judgment**
> seat gemma-4-E4B QAT q4_0 scores 21/22 at **1.24 s** against Qwen3-4B-thinking's
> 21/22 at **18.75 s** — a 15× latency cut in a seat that is currently dormant.
> Separately, §5a's "~11 GB resident" is the three-model total; extract + judge
> alone peaks at 6.89 GB RSS.
