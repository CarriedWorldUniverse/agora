...
Traversal guards: max depth 6 from root; ≤2000 dirs and ≤20000 entries per root; ≤64 concurrent loads. Hidden dirs skipped *below* the root (the root itself may be hidden — that's how `.system`/`.agents` work). Symlinked dirs followed for User/Repo/Admin, ignored for System. Missing root = empty, no error; per-skill parse errors surfaced as warnings (System scope errors silently dropped).
