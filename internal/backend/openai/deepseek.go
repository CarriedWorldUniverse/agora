...
func (p *Provider) responseFormatFor(rf *backend.ResponseFormat) *openai.ChatCompletionNewParamsResponseFormatUnion {
	if rf == nil {
		return nil
	}
	if p.isDeepSeek() && rf.Type == "json_schema" {
		// Degrade to json_object — the portable shape DeepSeek supports.
		return toOpenAIResponseFormat(&backend.ResponseFormat{Type: "json_object"})
	}
...
func classifyOpenAIError(err error) *backend.ProviderError {
	if err == nil {
		return nil
	}
...
func kindForStatus(status int) backend.ProviderErrorKind {
	switch status {
	case http.StatusTooManyRequests: // 429
		return backend.ProviderErrorRateLimit
	case http.StatusUnauthorized, http.StatusForbidden: // 401, 403
		return backend.ProviderErrorAuthFailed
	case http.StatusGatewayTimeout, http.StatusRequestTimeout: // 504, 408
		return backend.ProviderErrorTimeout
	}
	if status >= 500 {
		return backend.ProviderErrorServerError
	}
