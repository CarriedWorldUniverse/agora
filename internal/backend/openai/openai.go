...
import (
	...
	backend "github.com/CarriedWorldUniverse/agora/internal/backend"
	"github.com/CarriedWorldUniverse/agora/internal/backend/normalize"
)

// Provider implements backend.Provider for the OpenAI API.
...
func (p *Provider) Name() backend.ProviderID { return backend.ProviderOpenAI }

func (p *Provider) Capabilities() backend.ProviderCapabilities {
	return backend.ProviderCapabilities{
		Category:               backend.CategoryDirectAPI,
...
func (p *Provider) RunTurn(ctx context.Context, req backend.ProviderRequest, sink backend.EventSink) (backend.ProviderResult, error) {
	...
	for stream.Next() {
		...
		if !acc.AddChunk(chunk) {
			return backend.ProviderResult{}, fmt.Errorf("openai: accumulator rejected chunk")
		}
		...
		if len(chunk.Choices) > 0 && chunk.Choices[0].Delta.Content != "" {
			sink.Emit(backend.ModelChunk{Text: chunk.Choices[0].Delta.Content})
		}
	...
	if err := stream.Err(); err != nil {
		...
		if pe := classifyOpenAIError(err); pe != nil {
			return backend.ProviderResult{}, pe
		}
		return backend.ProviderResult{}, fmt.Errorf("openai: API error: %w", err)
	}
...
// backend.ProviderResult.
func extractResult(completion *openai.ChatCompletion, streamReasoning string) (backend.ProviderResult, error) {
	if len(completion.Choices) == 0 {
		return backend.ProviderResult{StopReason: backend.StopReasonModelDone}, nil
	}
	...
	var toolCalls []backend.ToolInvocation
	var sessionDelta []backend.SessionEvent

	if msg.Content != "" {
		backend.AppendAssistantText(&finalText, &sessionDelta, backend.ProviderOpenAI, msg.Content)
	}

	for _, tc := range msg.ToolCalls {
		argsJSON := json.RawMessage(tc.Function.Arguments)
		toolCalls = append(toolCalls, backend.ToolInvocation{
			ID:   tc.ID,
			Name: tc.Function.Name,
			Args: argsJSON,
		})
		raw, _ := json.Marshal(tc)
		sessionDelta = append(sessionDelta, backend.SessionEvent{
	...
	if reasoning != "" {
		...
		for i := range sessionDelta {
			if sessionDelta[i].Role == backend.RoleAssistant {
		...
		if !attached {
			...
			sessionDelta = append([]backend.SessionEvent{{
				Provider:         backend.ProviderOpenAI,
				Role:             backend.RoleAssistant,
...
func toOpenAIMessages(systemPrompt string, msgs []backend.ProviderMessage) []openai.ChatCompletionMessageParamUnion {
...
func toOpenAITools(defs []backend.ToolDef) []openai.ChatCompletionToolParam {
	if len(defs) == 0 {
		return nil
	}
...
// NEX-299 Pass 2.
func toOpenAIResponseFormat(rf *backend.ResponseFormat) *openai.ChatCompletionNewParamsResponseFormatUnion {
	if rf == nil {
		return nil
	}
	switch rf.Type {
...
// ToolChoice entirely even though backend.ProviderRequest carried it.
