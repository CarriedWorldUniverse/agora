...
func TestFS_ClassifyNativeShapeCarriesThePath(t *testing.T) {
	...
	// Assert on the TYPED field, not on stringified JSON: marshalling
	// escapes backslashes, so a substring check against a Windows path
	// (C:\Users\... becomes C:\\Users\\...) never matches even when the
	// payload is correct.
	pp, ok := payload.(PatchPayload)
	if !ok {
		t.Fatalf("write payload = %#v; want PatchPayload", payload)
	}
	if pp.Path != target {
		t.Fatalf("write payload Path = %q; want the target %q", pp.Path, target)
	}
	...
	rp, ok := payload.(ReadPayload)
	if !ok {
		t.Fatalf("read payload = %#v; want ReadPayload", payload)
	}
	if rp.Detail != target {
		t.Fatalf("read payload Detail = %q; want the target %q", rp.Detail, target)
	}
}
...
func TestFS_ClassifyNativeWriteOutsideRootsStillEscalates(t *testing.T) {
	// Temp-free roots + an absolute path outside every root: since the
	// sandbox gained scratch/tmp (#130), t.TempDir() is INSIDE a temp root
	// and is no longer an escape. Same reasoning as newTestRootsNoTemp's
	// doc comment — this test is about which ARGUMENT the containment
	// check reads, not about temp policy.
	roots := newTestRootsNoTemp(t)
	outside := absOutsideRoots()
